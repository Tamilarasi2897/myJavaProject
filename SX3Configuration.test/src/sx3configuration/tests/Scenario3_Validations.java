...
public class Scenario3_Validations extends TestFXBase {
	...
	@Test
	public void test1_scenario3_NewConfigurationwithvalidvalues() throws InterruptedException {
	...
	private void updateFormatResolutionTable() {
		...
		for (FormatAndResolutions formatAndResolutions : format_RESOLUTIONS) {
			...
			//			for (SensorConfig name : resolution_REGISTER_SETTING) {
			...
			if(videoSourceConfig1.getVIDEO_SOURCE_SUBTYPE().equals("HDMI RX - Generic")) {
				...
				for (HDMISourceConfiguration hdmiSourceConfiguration : hdmi_SOURCE_CONFIGURATION) {
					...
					for (SensorConfig hdmiSensorConfig : resolution_REGISTER_SETTING) {
						...
						//						write("{" + hdmiSensorConfig.getREGISTER_ADDRESS() + ",").push(KeyCode.TAB);
						//						write(hdmiSensorConfig.getREGISTER_VALUE() + ",").push(KeyCode.TAB);
						//						write(hdmiSensorConfig.getSLAVE_ADDRESS() + "},").push(KeyCode.TAB);
					}
	...
	private void updateSlaveFifoSettings() {
		...
		String endPoint1TabName = "Endpoint 1 -" + sx3Obj.getDEVICE_SETTINGS().getUSB_SETTINGS().getENDPOINT1_TYPE();
		clickOn(endPoint1TabName);
		// clickOn("#slaveFifoSettingsBrustLength");
		// String burstlength =
		// Integer.toString(sx3Obj.getSLAVE_FIFO_SETTINGS().get(0).getBURST_LENGTH());
		// write(burstlength).push(KeyCode.TAB);
		...
		// clickOn("#slaveFifoSettingsBufferCount");
	...
	private void updateFifoMasterConfig() {
		...
		if (sx3Obj.getFIFO_MASTER_CONFIG().getFIFO_MASTER_CONFIGURATION_DOWNLOAD_EN().equals("Enable")) {
			handle_combo_with_string("#fpgaFamily", sx3Obj.getFIFO_MASTER_CONFIG().getFPGA_FAMILY());
			...
			if (!sx3Obj.getFIFO_MASTER_CONFIG().getBIT_FILE_PATH().isEmpty()) {
				...
				File source=new File(loadedConfigurationPath.getParent()+"\\"+bitFileName);
				...
				//				update_text_field("#bitFileSize", Integer.toString(sx3Obj.getFIFO_MASTER_CONFIG().getBIT_FILE_SIZE()));
				//				File file = new File(sx3Obj.getFIFO_MASTER_CONFIG().getBIT_FILE_PATH());
				//				if(file.exists()) {
				System.out.println("bit file size"+String.valueOf((sx3Obj.getFIFO_MASTER_CONFIG().getBIT_FILE_SIZE())));
				//((TextField) lookupControl("#bitFileSize")).setText(String.valueOf(file.length()));
				((TextField) lookupControl("#bitFileSize")).setText(String.valueOf((sx3Obj.getFIFO_MASTER_CONFIG().getBIT_FILE_SIZE()))); 			
			}
		}
	...
	private void createNewConfiguration() {
		...
		write(sx3Obj.getDEVICE_NAME()).clickOn(sx3Obj.getDEVICE_NAME()).push(KeyCode.TAB);
	...
	private void loadConfiguration() {
		...
		clickOn(LOADCONFIG_SELECTION); sleep(2000);
		...
		System.out.println("newConfigurationFileName===>"+newConfigurationFilePath + "\\" + newConfigurationFileName+"\\"+newConfigurationFileName+".json");

		/*((TextField) lookupControl("#sx3ConfigurationToImport"))
	.setText(PREDEFINED_IMPORT_FROM); sleep(2000);*/

		//System.out.println("PREDEFINED_IMPORT_FROM===>"+PREDEFINED_IMPORT_FROM);
	...
	private void clear_textfield(TextField textField) {
